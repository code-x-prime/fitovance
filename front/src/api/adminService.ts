...
// Reels / Watch & Buy
export const reels = {
  getReels: () => api.get("/api/admin/reels"),
  getReelById: (id: string) => api.get(`/api/admin/reels/${id}`),
  createReel: (formData: FormData) =>
    api.post("/api/admin/reels", formData, {
      headers: { "Content-Type": "multipart/form-data" },
    }),
  updateReel: (id: string, formData: FormData) =>
    api.put(`/api/admin/reels/${id}`, formData, {
      headers: { "Content-Type": "multipart/form-data" },
    }),
  deleteReel: (id: string) => api.delete(`/api/admin/reels/${id}`),
  reorderReels: (reelIds: string[]) =>
    api.patch("/api/admin/reels/reorder", { reelIds }),
  bulkUpdateStatus: (reelIds: string[], status: "ACTIVE" | "INACTIVE") =>
    api.patch("/api/admin/reels/bulk/status", { reelIds, status }),
  bulkDelete: (reelIds: string[]) =>
    api.post("/api/admin/reels/bulk/delete", { reelIds }),
};
