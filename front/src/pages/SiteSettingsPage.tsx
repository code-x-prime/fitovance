...
export default function SiteSettingsPage() {
  ...
  const [showOAuthSecrets, setShowOAuthSecrets] = useState<Record<string, boolean>>({});
          ...
          <Card className="bg-[var(--bg-card)] border-[var(--border-color)]">
            <CardHeader>
              ...
              <p className="text-xs text-blue-600 bg-blue-50 p-3 rounded border border-blue-200">
                <strong>Facebook:</strong> In Meta for Developers → App Settings → Valid OAuth Redirect URIs, add: <code className="bg-blue-100 px-1 rounded block mt-1">{(import.meta.env.VITE_API_URL || "http://localhost:4001").replace(/\/$/, "")}/api/auth/facebook/callback</code>
              </p>
              <p className="text-xs text-sky-600 bg-sky-50 p-3 rounded border border-sky-200">
                <strong>Twitter:</strong> In Twitter Developer Portal → App Settings → User authentication settings → Callback URI, add: <code className="bg-sky-100 px-1 rounded block mt-1">{(import.meta.env.VITE_API_URL || "http://localhost:4001").replace(/\/$/, "")}/api/auth/twitter/callback</code>
              </p>
            </CardHeader>
                      ...
                      <Switch
                        ...
                        onCheckedChange={async (c) => {
                          ...
                          try {
                            ...
                            toast.success(`${provider.charAt(0).toUpperCase() + provider.slice(1)} ${c ? "enabled" : "disabled"}`);
                            fetchOAuthSettings();
                          } catch (err: unknown) {
                            const msg = err && typeof err === "object" && "response" in err ? (err as { response?: { data?: { message?: string } } }).response?.data?.message : undefined;
                            toast.error(msg || "Failed to save");
                    ...
                    {data.isEnabled && (
                      <>
                        ...
                        <div>
                          ...
                          <div className="relative mt-1">
                            <Input
                              type={showOAuthSecrets[provider] ? "text" : "password"}
                            ...
                            <button
                              ...
                              onClick={() => setShowOAuthSecrets((s) => ({ ...s, [provider]: !s[provider] }))}
                            >
                              {showOAuthSecrets[provider] ? <EyeOff className="h-4 w-4" /> : <Eye className="h-4 w-4" />}
                            </button>
                          </div>
                          <p className="text-xs text-[var(--text-secondary)] mt-1">
